...
void TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
{
    ...
    if (mFindOpened)
    {
        ...
        ImGui::SetNextWindowPos(ImVec2(ImGui::GetWindowPos().x + ImGui::GetWindowWidth()*0.25, ImGui::GetWindowPos().y + ImGui::GetWindowHeight()*0.5 - 60), ImGuiCond_Appearing);
        ImGui::SetNextWindowSize(ImVec2(ImGui::GetWindowWidth()*0.5, 120), ImGuiCond_Appearing);

        ImGui::Begin("Find and Replace", &mFindOpened, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse);
        ...
        if (!mReadOnly) {
            ...
            ImGui::SameLine();
            if (ImGui::Button("Replace") || shouldReplace) {
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::SimpleConsole()
{
    ...
    {
        static const char* const keywords[] = {
            "help","listfiles","apimethods","exit","loop","MOUSE_X","MOUSE_Y","OUTPUT_WIDTH","OUTPUT_HEIGHT",
        ...
        //langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("[a-zA-Z_]-[a-zA-Z0-9_]*", PaletteIndex::Identifier));
        langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("[a-zA-Z-_]+", PaletteIndex::Identifier));
